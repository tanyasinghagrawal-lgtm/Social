...
class User(Base):
    ...
    follower_count = Column(Integer, default=0)   # NEW
    following_count = Column(Integer, default=0)  # NEW
...
async def get_followers(user_id: int):
    # Fetch latest 20 followers directly from DB
    async with AsyncSessionLocal() as session:
        stmt = (
            select(User)
...
async def get_following(user_id: int):
    # Fetch latest 20 users that target_id is following directly from DB
    async with AsyncSessionLocal() as session:
        stmt = (
            select(User)
