...
from fastapi import FastAPI, HTTPException, Depends, BackgroundTasks, Query, status, Header
...
async def get_verified_user_id(x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")) -> int:
    """
    Validates Telegram WebApp initData HMAC-SHA256 signature using BOT_TOKEN.
    Runs directly on the event loop for ultra-low latency without threadpool overhead.
    """    
    if not x_telegram_init_data:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Telegram authentication required (Missing initData)"
        )
    try:
        ...
        if not received_hash:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Invalid Telegram data (Missing hash)"
            )

        # 1. Alphabetically sort and join with newline
        data_check_string = "\n".join(f"{k}={v}" for k, v in sorted(parsed_data.items()))

        # 2. Secret key: HMAC-SHA256("WebAppData", BOT_TOKEN)
        secret_key = hmac.new(b"WebAppData", BOT_TOKEN.encode(), hashlib.sha256).digest()

        # 3. Calculate hash of data_check_string
        ...
        if not hmac.compare_digest(calculated_hash, received_hash):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Unauthorized: Telegram signature verification failed"
            )

        # 5. Extract and return verified user ID
        user_data = json.loads(parsed_data.get("user", "{}"))
        user_id = user_data.get("id")
        if not user_id:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Invalid user info inside Telegram initData"
            )
        return int(user_id)
...
@app.get("/update")
async def check_live_updates(telegram_id: int = Depends(get_verified_user_id), since: float = Query(0.0)):    
...
@app.post("/newpost")
async def new_post(payload: NewPostRequest, background_tasks: BackgroundTasks, verified_user_id: int = Depends(get_verified_user_id)):
    payload.telegram_id = verified_user_id
    today = datetime.utcnow().strftime("%Y-%m-%d")    
...
@app.post("/like")
async def like_post(payload: LikeRequest, background_tasks: BackgroundTasks, verified_user_id: int = Depends(get_verified_user_id)):
    payload.telegram_id = verified_user_id
    async with AsyncSessionLocal() as session:        
        await update_user_activity(session, payload.telegram_id)
        ...
        # 💡 NEW: Track Like as an Interaction
        background_tasks.add_task(redis_client.hincrby, f"user:{payload.telegram_id}:author_clicks", str(post.author_telegram_id), 1)
        return {"status": 200, "message": "Post liked successfully"}
@app.post("/unlike")
async def unlike_post(payload: LikeRequest, background_tasks: BackgroundTasks, verified_user_id: int = Depends(get_verified_user_id)):
    payload.telegram_id = verified_user_id
    async with AsyncSessionLocal() as session:        
        await update_user_activity(session, payload.telegram_id)
...
@app.post("/impression")
async def record_impression(payload: ImpressionRequest, background_tasks: BackgroundTasks, verified_user_id: int = Depends(get_verified_user_id)):
    payload.viewed_by = verified_user_id
    ...
    async with AsyncSessionLocal() as session:
        ...
        if post:
            ...
            background_tasks.add_task(update_post_cache_stats, payload.post_id, "impression_count", 1) # 💡 NEW: Update feed impression in cache
...
@app.get("/loadpost")
async def load_post(post_id: int, telegram_id: int = Depends(get_verified_user_id), background_tasks: BackgroundTasks = None):
    async with AsyncSessionLocal() as session:
        ...
        # 💡 NEW: Track author clicks for recommendation boost
        if post_data["author_id"] != telegram_id:
            background_tasks.add_task(redis_client.hincrby, f"user:{telegram_id}:author_clicks", str(post_data["author_id"]), 1)
            background_tasks.add_task(redis_client.expire, f"user:{telegram_id}:author_clicks", 604800) # Save for 7 days
...
@app.get("/homefeed")
async def home_feed(telegram_id: int = Depends(get_verified_user_id), cursor_post_id: Optional[int] = Query(None), background_tasks: BackgroundTasks = None):    
    async with AsyncSessionLocal() as session:
        await update_user_activity(session, telegram_id)
        
        # 1. Target User Blocked List Fetch
        blocked_res = await session.execute(
            select(Block).where(or_(Block.blocker_id == telegram_id, Block.blocked_id == telegram_id))
        )
        blocked_ids = set()
        ...
        # 2. Vector Embeddings
        # 2. Vector Embeddings (Last 50 Likes & Comments)
        # 2. Vector Embeddings (Optimized with 15-min Redis Cache for ultra-fast scrolling)
        vector_cache_key = f"user:{telegram_id}:vector"
        cached_vector = await redis_client.get(vector_cache_key)
        
        if cached_vector:
            # Scroll karte waqt cache se instant load hoga
            user_vector = json.loads(cached_vector)
        else:
            # Pehli baar feed kholne par DB se compute hoga (Last 50 likes & comments)
            interacted_posts = await session.execute(
                select(Post.embedding)
                .where(
                    or_(
                        Post.id.in_(select(Like.post_id).where(Like.user_telegram_id == telegram_id)),
                        Post.id.in_(select(Comment.post_id).where(Comment.author_telegram_id == telegram_id))
                    )
                )
                .order_by(Post.id.desc())
                .limit(50)
            )
            embeddings = [e for e in interacted_posts.scalars().all() if e and len(e) == 128]
            user_vector = [sum(e[i] for e in embeddings) / len(embeddings) for i in range(128)] if embeddings else None
            
            # Vector banne ke baad usko Redis me 15 minutes (900 seconds) ke liye save kar do
            if user_vector:
                await redis_client.set(vector_cache_key, json.dumps(user_vector), ex=900)        
    ...
    # 4. Fetch Personalized Seen Posts & Author Clicks from Redis (Ultra-fast)
    pipe = redis_client.pipeline()
    pipe.smembers(f"user:{telegram_id}:seen_posts")
    pipe.hgetall(f"user:{telegram_id}:author_clicks")
    r_data = await pipe.execute()
    seen_posts = {int(x) for x in r_data[0]} if r_data[0] else set()
    author_clicks = {int(k): int(v) for k, v in r_data[1].items()} if r_data[1] else {}

    # --- 5. Redis In-Memory Candidate Retrieval (Increased to 100) ---
    candidate_posts = []
    missing_ids = []
    ...
    try:
        ...
        post_ids = await redis_client.zrevrangebyscore("posts:recent:zset", max_score, "-inf", start=0, num=100)
    ...
    # --- 6. Fallback to DB ---
    if len(candidate_posts) < 20 or missing_ids:
        async with AsyncSessionLocal() as session:
            query = select(Post, User.username).outerjoin(User, Post.author_telegram_id == User.telegram_id)
            if cursor_post_id:
                query = query.where(Post.id < cursor_post_id)
            query = query.order_by(Post.id.desc()).limit(100)
            ...
            for p, u_name in db_candidates:
                if not any(cp["post_id"] == p.id for cp in candidate_posts):
                    p_data = {
                        "post_id": p.id, "author_id": p.author_telegram_id,
                        "author_username": u_name or "anonymous", "snippet": p.content[:70],
                        "content": p.content, "embedding": p.embedding or [],
                        "like_count": p.like_count, "comment_count": p.comment_count,
                        "view_count": p.view_count, "impression_count": p.impression_count, # 💡 NEW
                        "created_at": p.created_at.isoformat()
                    }
    ...
    # 7. Filter out Seen Posts & Blocked Users
    filtered_candidates = [
        cp for cp in candidate_posts 
        if cp["post_id"] not in seen_posts and cp["author_id"] not in blocked_ids
    ]

    # 8. Ultra-Advanced 2-Step Funnel Scoring Algorithm
    scored_posts = []
    for cp in filtered_candidates:
        sim = cosine_similarity(user_vector, cp.get("embedding")) if user_vector else 0.0
        
        raw_views = cp.get("view_count", 0)
        # Fallback if impression_count is missing from older cached posts
        feed_impressions = cp.get("impression_count", max(raw_views - 1, 0)) 
        likes = cp.get("like_count", 0)
        comments = cp.get("comment_count", 0)
        
        # Exact Metrics Breakdown
        clicks = max(raw_views - feed_impressions, 0)
        feed_impressions = max(feed_impressions, 1) # Prevent zero division
        
        # RATIO 1: Click-Through Ratio (CTR) (+50 Bayesian smoother)
        ctr_ratio = clicks / (feed_impressions + 50)
        
        # RATIO 2: Action Ratio (+20 Bayesian smoother)
        action_ratio = ((likes * 2) + (comments * 3)) / (clicks + 20)
        
        # Total Popularity: Combined percentage (No raw numbers added)
        popularity = (ctr_ratio * 100) + (action_ratio * 100)
        
        final_score = (sim * 10.0) + (popularity * 0.2) + random.uniform(0.0, 1.0)
        
        # Follower Boost
        if cp.get("author_id") in followed_ids:
            final_score *= 1.20 
            
        # Heavy Affinity Boost: Agar is user ne same creator ko >2 baar click/like/comment kiya hai
        # Heavy Affinity Boost: Agar is user ne same creator ko >2 baar click/like/comment kiya hai
        if author_clicks.get(int(cp.get("author_id")), 0) >= 2:
            final_score *= 1.40
        # True Trending Boost: Sirf quality engagement pe
        if ctr_ratio > 0.10 and action_ratio > 0.10 and feed_impressions > 50:
            final_score *= 1.30

        scored_posts.append((final_score, cp))

    # Sort and pick exactly NEXT 16 posts
    scored_posts.sort(key=lambda x: x[0], reverse=True)
    selected_posts = [sp[1] for sp in scored_posts[:16]]
    
    # 9. Background Task: Add these 16 posts to 'seen_posts' so they don't repeat
    if selected_posts and background_tasks:
        sent_ids = [p["post_id"] for p in selected_posts]
        background_tasks.add_task(redis_client.sadd, f"user:{telegram_id}:seen_posts", *sent_ids)
        background_tasks.add_task(redis_client.expire, f"user:{telegram_id}:seen_posts", 259200) # Reset seen history every 3 days
    ...
    return {
        "posts": [
            {
                "post_id": p["post_id"], "author_id": p["author_id"],
                "author_username": p["author_username"], "snippet": p["snippet"],
...
@app.get("/profile")
async def get_profile(target_id: int, loaded_by: int = Depends(get_verified_user_id)):    
    async with AsyncSessionLocal() as session:
...
@app.post("/follow")
async def follow_user(payload: FollowRequest, background_tasks: BackgroundTasks, verified_user_id: int = Depends(get_verified_user_id)):
    payload.follower_id = verified_user_id
...
@app.post("/unfollow")
async def unfollow_user(payload: FollowRequest, verified_user_id: int = Depends(get_verified_user_id)):
    payload.follower_id = verified_user_id
    async with AsyncSessionLocal() as session:        
        await update_user_activity(session, payload.follower_id)
...
@app.post("/report")
async def report_content(payload: ReportRequest, verified_user_id: int = Depends(get_verified_user_id)):
    payload.reported_by = verified_user_id
    async with AsyncSessionLocal() as session:
...
@app.post("/delete")
async def delete_item(payload: ItemDeleteRequest, background_tasks: BackgroundTasks, verified_user_id: int = Depends(get_verified_user_id)):
    payload.telegram_id = verified_user_id
    async with AsyncSessionLocal() as session:        
        await update_user_activity(session, payload.telegram_id)
...
async def delete_profile(telegram_id: int = Depends(get_verified_user_id)):
    async with AsyncSessionLocal() as session:        
        user = await session.get(User, telegram_id)
        if not user:
...
async def send_dm(payload: SendMessageRequest, background_tasks: BackgroundTasks, verified_user_id: int = Depends(get_verified_user_id)):
    payload.sender_id = verified_user_id
    async with AsyncSessionLocal() as session:        
...
@app.get("/getmessages")
async def get_messages(user2: int, user1: int = Depends(get_verified_user_id), cursor_msg_id: Optional[int] = Query(None)):
    async with AsyncSessionLocal() as session:
...
async def get_conversations(telegram_id: int = Depends(get_verified_user_id), cursor_msg_id: Optional[int] = Query(None)):
    async with AsyncSessionLocal() as session:
        await update_user_activity(session, telegram_id)        
...
@app.post("/block")
async def block_user(payload: BlockRequest, verified_user_id: int = Depends(get_verified_user_id)):
    payload.blocker_id = verified_user_id
    async with AsyncSessionLocal() as session:
...
@app.post("/unblock")
async def unblock_user(payload: BlockRequest, verified_user_id: int = Depends(get_verified_user_id)):
    payload.blocker_id = verified_user_id
    async with AsyncSessionLocal() as session:
        await update_user_activity(session, payload.blocker_id)        
...
@app.post("/new")
async def create_username(payload: UsernameRequest, background_tasks: BackgroundTasks, verified_user_id: int = Depends(get_verified_user_id)):
    payload.telegram_id = verified_user_id
    async with AsyncSessionLocal() as session:        
        await update_user_activity(session, payload.telegram_id)
...
async def update_avatar(payload: AvatarUpdateRequest, verified_user_id: int = Depends(get_verified_user_id)):
    payload.telegram_id = verified_user_id
    async with AsyncSessionLocal() as session:
        await update_user_activity(session, payload.telegram_id)
        user = await session.get(User, payload.telegram_id)        
...
async def update_gender(payload: GenderUpdateRequest, verified_user_id: int = Depends(get_verified_user_id)):
    payload.telegram_id = verified_user_id
    async with AsyncSessionLocal() as session:
        user = await session.get(User, payload.telegram_id)        
        if not user:
...
@app.post("/comment")
async def post_comment(payload: CommentRequest, background_tasks: BackgroundTasks, verified_user_id: int = Depends(get_verified_user_id)):
    payload.telegram_id = verified_user_id
    async with AsyncSessionLocal() as session:        
        await update_user_activity(session, payload.telegram_id)
        ...
        # 💡 NEW: Track Comment as an Interaction
        background_tasks.add_task(redis_client.hincrby, f"user:{payload.telegram_id}:author_clicks", str(post.author_telegram_id), 1)
        return {"status": 200, "message": "Comment posted"}
@app.get("/notification")
async def get_notifications(telegram_id: int = Depends(get_verified_user_id), cursor_notif_id: Optional[int] = Query(None)):   
    async with AsyncSessionLocal() as session:
        await update_user_activity(session, telegram_id)
...
async def read_notifications(payload: ReadNotificationsRequest, verified_user_id: int = Depends(get_verified_user_id)):
    payload.telegram_id = verified_user_id
    async with AsyncSessionLocal() as session:
