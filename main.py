...
async def get_gemini_embedding(text: str) -> List[float]:
    """Tries all 4 decoded Gemini API keys sequentially with 128 dimensions and alerts Admin if all fail."""
    errors = []
        ...
        for idx, key in enumerate(GEMINI_KEYS):
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    err_msg = f"Key #{idx + 1} HTTP {res.status_code}: {res.text[:120]}"
                    logger.warning(f"Gemini embedding failed: {err_msg}")
                    errors.append(err_msg)
            except Exception as e:
                err_msg = f"Key #{idx + 1} Exception: {str(e)}"
                logger.warning(f"Gemini embedding error: {err_msg}")
                errors.append(err_msg)

    # Agar charo keys fail ho jayein toh Admin ko alert bhejna
    if errors:
        error_lines = "\n".join([f"• <code>{err}</code>" for err in errors])
        admin_alert_text = (
            f"⚠️ <b>Gemini Embedding Failure Alert (128-dim)!</b>\n\n"
            f"📝 <b>Post Text:</b> <i>\"{text[:60]}...\"</i>\n\n"
        ...
        try:
            ...
            logger.error(f"Failed to send admin embedding error alert: {notify_err}")
