...
async def home_feed(telegram_id: int, cursor_post_id: Optional[int] = Query(None), background_tasks: BackgroundTasks = None):
    async with AsyncSessionLocal() as session:
        ...
        return {
            ...
            "last_post_id": selected_posts[-1].id if selected_posts else None
        }
...
async def delete_profile(telegram_id: int):
    async with AsyncSessionLocal() as session:
        ...
        await session.execute(delete(Notification).where(Notification.user_id == telegram_id))
...
@app.post("/new")
async def create_username(payload: UsernameRequest):
    async with AsyncSessionLocal() as session:
        await update_user_activity(session, payload.telegram_id)
